...
public class ChatController {
        ...
        public ResponseEntity<GlobalResponse<ChatSessionResponse>> createSession(
                        @RequestParam UUID userId,
                        @RequestParam(required = false) String mode,
                        HttpServletRequest servletRequest) {

                com.techhub.app.aiservice.enums.ChatMode chatMode = com.techhub.app.aiservice.enums.ChatMode.GENERAL;
                if (mode != null) {
                        try {
                                chatMode = com.techhub.app.aiservice.enums.ChatMode.valueOf(mode.toUpperCase());
                        } catch (IllegalArgumentException e) {
                                chatMode = com.techhub.app.aiservice.enums.ChatMode.GENERAL;
                        }
                }

                ChatSessionResponse response = chatOrchestrationService.createSession(userId, chatMode);
                return ResponseEntity.ok(
                                GlobalResponse.success("Session created", response)
                                                .withStatus("SUCCESS")
                                                .withPath(servletRequest.getRequestURI()));
        }
